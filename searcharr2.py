...
# Lidarr Support
        self.lidarr = (
            lidarr.Lidarr(settings.lidarr_url, settings.lidarr_api_key, args.verbose)
            if settings.lidarr_enabled
            else None
        )
        if self.lidarr:
            quality_profiles = []
            if not isinstance(settings.lidarr_quality_profile_id, list):
                settings.lidarr_quality_profile_id = [
                    settings.lidarr_quality_profile_id
                ]
            for i in settings.lidarr_quality_profile_id:
                logger.debug(
                    f"Looking up/validating lidarr quality profile id for [{i}]..."
                )
                foundProfile = self.lidarr.lookup_quality_profile(i)
                if not foundProfile:
                    logger.error(f"lidarr quality profile id/name [{i}] is invalid!")
                else:
                    logger.debug(
                        f"Found lidarr quality profile for [{i}]: [{foundProfile}]"
                    )
                    quality_profiles.append(foundProfile)
            if not len(quality_profiles):
                logger.warning(
                    f"No valid lidarr quality profile(s) provided! Using all of the quality profiles I found in lidarr: {self.lidarr._quality_profiles}"
                )
            else:
                logger.debug(
                    f"Using the following lidarr quality profile(s): {[(x['id'], x['name']) for x in quality_profiles]}"
                )
                self.lidarr._quality_profiles = quality_profiles
            metadata_profiles = []
            if not isinstance(settings.lidarr_metadata_profile_id, list):
                settings.lidarr_metadata_profile_id = [
                    settings.lidarr_metadata_profile_id
                ]
            for i in settings.lidarr_metadata_profile_id:
                logger.debug(
                    f"Looking up/validating lidarr metadata profile id for [{i}]..."
                )
                foundProfile = self.lidarr.lookup_metadata_profile(i)
                if not foundProfile:
                    logger.error(f"lidarr metadata profile id/name [{i}] is invalid!")
                else:
                    logger.debug(
                        f"Found lidarr metadata profile for [{i}]: [{foundProfile}]"
                    )
                    metadata_profiles.append(foundProfile)
            if not len(metadata_profiles):
                logger.warning(
                    f"No valid lidarr metadata profile(s) provided! Using all of the metadata profiles I found in lidarr: {self.lidarr._metadata_profiles}"
                )
            else:
                logger.debug(
                    f"Using the following lidarr metadata profile(s): {[(x['id'], x['name']) for x in metadata_profiles]}"
                )
                self.lidarr._metadata_profiles = metadata_profiles

            root_folders = []
            if not hasattr(settings, "lidarr_music_paths"):
                settings.lidarr_music_paths = []
                logger.warning(
                    'No lidarr_movie_paths setting detected. Please set one in settings.py (lidarr_music_paths=["/path/1", "/path/2"]). Proceeding with all root folders configured in lidarr.'
                )
            if not isinstance(settings.lidarr_music_paths, list):
                settings.lidarr_music_paths = [settings.lidarr_music_paths]
            for i in settings.lidarr_music_paths:
                logger.debug(f"Looking up/validating lidarr root folder for [{i}]...")
                foundPath = self.lidarr.lookup_root_folder(i)
                if not foundPath:
                    logger.error(f"lidarr root folder path/id [{i}] is invalid!")
                else:
                    logger.debug(f"Found lidarr root folder for [{i}]: [{foundPath}]")
                    root_folders.append(foundPath)
            if not len(root_folders):
                logger.warning(
                    f"No valid lidarr root folder(s) provided! Using all of the root folders I found in lidarr: {self.lidarr._root_folders}"
                )
            else:
                logger.debug(
                    f"Using the following lidarr root folder(s): {[(x['id'], x['path']) for x in root_folders]}"
                )
                self.lidarr._root_folders = root_folders
            if not hasattr(settings, "lidarr_tag_with_username"):
                settings.lidarr_tag_with_username = True
                logger.warning(
                    "No lidarr_tag_with_username setting found. Please add lidarr_tag_with_username to settings.py (lidarr_tag_with_username=True or lidarr_tag_with_username=False). Defaulting to True."
                )
            if not hasattr(settings, "lidarr_music_command_aliases"):
                settings.lidarr_music_command_aliases = ["music"]
                logger.warning(
                    'No lidarr_music_command_aliases setting found. Please add lidarr_movie_command_aliases to settings.py (e.g. lidarr_music_command_aliases=["book", "bk"]. Defaulting to ["book"].'
                )
            if not hasattr(settings, "lidarr_forced_tags"):
                settings.lidarr_forced_tags = []
                logger.warning(
                    'No lidarr_forced_tags setting found. Please add lidarr_forced_tags to settings.py (e.g. lidarr_forced_tags=["tag-1", "tag-2"]) if you want specific tags added to each movie. Defaulting to empty list ([]).'
                )
            if not hasattr(settings, "lidarr_allow_user_to_select_tags"):
                settings.lidarr_allow_user_to_select_tags = True
                logger.warning(
                    "No lidarr_allow_user_to_select_tags setting found. Please add lidarr_allow_user_to_select_tags to settings.py (e.g. lidarr_allow_user_to_select_tags=False) if you do not want users to be able to select tags when adding a movie. Defaulting to True."
                )
            if not hasattr(settings, "lidarr_user_selectable_tags"):
                settings.lidarr_user_selectable_tags = []
                logger.warning(
                    'No lidarr_user_selectable_tags setting found. Please add lidarr_user_selectable_tags to settings.py (e.g. lidarr_user_selectable_tags=["tag-1", "tag-2"]) if you want to limit the tags a user can select. Defaulting to empty list ([]), which will present the user with all tags.'
                )
            for t in settings.lidarr_user_selectable_tags:
                if t_id := self.lidarr.get_tag_id(t):
                    logger.debug(
                        f"Tag id [{t_id}] for user-selectable lidarr tag [{t}]"
                    )
            for t in settings.lidarr_forced_tags:
                if t_id := self.lidarr.get_tag_id(t):
                    logger.debug(f"Tag id [{t_id}] for forced lidarr tag [{t}]")
# End Lidarr Support --
...
# Lidarr Commands
    ...
    def cmd_album(self, update, context):
        logger.debug(f"Received artist cmd from [{update.message.from_user.username}]")
        if not self._authenticated(update.message.from_user.id):
            update.message.reply_text(
                self._xlate(
                    "auth_required",
        ...
        if not settings.lidarr_enabled:
            update.message.reply_text(self._xlate("lidarr_disabled"))
            return
        title = self._strip_entities(update.message)
        if not len(title):
            x_title = self._xlate("title").title()
            update.message.reply_text(
                self._xlate(
                    "include_artist_title_in_cmd",
                    commands=" OR ".join(
                        [
                            f"`/{c} {x_title}`"
                            for c in settings.lidarr_artist_command_aliases
                        ]
                    ),
                )
            )
    ...
    def callback(self, update, context):
        ...
        if op == "noop":
            ...
            if convo["type"] in ["series", "movie", "book", "album", "artist"]:
                if i <= 0:
                    query.answer()
                    return
                r = convo["results"][i - 1]
            ...
            if convo["type"] in ["series", "movie", "book", "artist", "album"]:
            ...
            paths = (
                ...
                if convo["type"] == "book"
                else self.lidarr._root_folders
                if convo["type"] == "artist"
                #if convo["type"] == "album"
                else []
            )
            ...
            if not additional_data.get("q"):
                quality_profiles = (
                    ...
                    if convo["type"] == "book"
                    else self.lidarr._quality_profiles
                )
    ...
    def cmd_help(self, update, context):
        ...
        if self.lidarr:
            lidarr_help = self._xlate(
                "help_lidarr",
                music_commands=" OR ".join(
                    [
                        f"`/{c} {self._xlate('title').title()}`"
                        for c in settings.lidarr_music_command_aliases
                    ]
                ),
            )    
    ...
    def run(self):
        ...
        if self.readarr:
            ...
            for c in settings.lidarr_music_command_aliases:
                logger.debug(f"Registering [/{c}] as a music command")
                updater.dispatcher.add_handler(CommandHandler(c, self.cmd_music))
        for c in settings.radarr_movie_command_aliases:
